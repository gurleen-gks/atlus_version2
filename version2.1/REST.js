...
function showNodes(req, res)
{
    ...
    db.query("select f1.csha as curr, f2.csha as prev,cts.cwhen as cwhen,cts.email as email, (f1." + metric + "-f2." + metric + ") as " + metric + " from  ${table~} as f1, " +
        table + " as f2,cpairs as cpairs, commits as cts where cts.application=f1.application and f1.application=${app} and f2.application=f1.application and f1.application=cpairs.application and f1.csha=cpairs.curr and f2.csha=cpairs.prev and cts.csha=f1.csha and " +
        filters + " order by cts.cwhen",{metric:metric, table:table, app:app})
            .then(function (data) {
            ...
            else
            {
                console.log("need login first");
                         res.status(200)
                             .json({
                                status: "fail",
                                message: "you need go to login page"
                             });
                         res.end();
            }
 ...
 //console.log(res);
        ...
        else
        {
             ...
             var pwd=req.body.pwd;
                var nstring="select password from registered_user where email='"+email+"'";        //SQL Injection can happen here, use PreparedStatement_gurleen!
             //var query = client.query(nstring);                                                   // demo to change to db(pg-promise connection)
             db.any(nstring).then(
             //query.on('row',
                 function(data) {
                    var row = data.length && data[0];
    ...
    router.get('/checkstatus', function (req,res) {
      if (req.session.sign) {
                         res.status(200)
                             .json({
                                status: "success",
                                message: "you are ready to query"
                             });
                         res.end();
        }  
        else
    ...
    router.get('/register',function(req,res){
        ...
        var client = new pg.Client(connectionString);
        client.connect();
        var nstring="select COUNT(email) as count from registered_user where email='"+eml+"'"; // Use prepared statement
        var query = client.query(nstring);
        query.on('row', function(row) {
            if(row.count==0)
             {
